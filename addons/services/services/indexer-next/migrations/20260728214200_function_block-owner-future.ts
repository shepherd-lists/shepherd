...
export async function up(knex: Knex): Promise<void> {
	await knex.raw(`
		CREATE OR REPLACE FUNCTION block_owner_future(owner_name VARCHAR(43))
...
export async function down(knex: Knex): Promise<void> {
	await knex.raw(`
		DROP FUNCTION IF EXISTS block_owner_future(owner_name VARCHAR(43));
	`)
}
