...
describe('mapVideoErrorResult', () => {
  test('no-stream maps to corrupt (with or without the "#0")', () => {
  ...
  test('known signatures map to corrupt-maybe', () => {
    const signatures = [
  ...
  test('unknown errors map to corrupt-maybe', () => {
    assert.equal(mapVideoErrorResult(new Error('mystery failure')).data_reason, 'corrupt-maybe')
  })

  test('reads the full stderr when the signature is not on the last line', () => {
    const stderr = [
...
describe('isRetryableVideoError', () => {
  test('ENOMEM/ECONNRESET retryable; corrupt signatures not', () => {
