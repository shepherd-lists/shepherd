...
test('loadPlugin loads and initializes the first plugin from the `plugins` array', async () => {
...
test('loadPlugin rejects a config that is missing the `plugins` array', async () => {
  const configPath = await writeConfig({})
  await assert.rejects(() => loadPlugin(configPath), /'plugins' must be a non-empty array/)
})

test('loadPlugin rejects an empty `plugins` array', async () => {
