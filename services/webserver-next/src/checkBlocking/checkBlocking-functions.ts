...
console.info(prefix, 'parse(RANGELIST_ALLOWED)', rangeItems)

const gwUrls: string[] = JSON.parse(process.env.GW_URLS || '[]')
console.info(prefix, 'gwUrls', gwUrls)

let _running = false
export const checkBlockedCronjob = async () => {

	console.info(prefix, `starting ${checkBlockedCronjob.name}() cronjob...`, { rangeItems, gwUrls, _running })
	if (_running) {
		console.info(prefix, `${checkBlockedCronjob.name}() already running. exiting...`)
		return
	}
	_running = true
	...
	try {
		...
		if (gwUrls.length === 0) {
			console.info(prefix, 'gwUrls empty. nothing to check txids against.')
		} else {
			...
			for await (const txid of txids) {
				...
				await Promise.all(gwUrls.map(async gw => {
					try {
						...
						slackLog(prefix, `gateway ${gw} is unreachable! while fetching ${gw}/${txid}`, txid)
					}
				}))
			}
		...
		if (gwUrls.length === 0 && rangeItems.length === 0) {
			console.info(prefix, 'gwUrls & accessRangelist empty. nothing to check byteranges against.')
			...
			for await (const range of ranges) {
				...
				await Promise.all(gwUrls.map(async gw => {
					try {
						...
						setUnreachable({ name: gw, server: gw })
						slackLog(prefix, `gateway ${gw} is unreachable! while fetching ${gw}/chunk/${+range1 + 1}`, range)
					}
				}))
				await Promise.all(rangeItems.map(async item => {
					try {
						...
						setUnreachable(item)
						console.info(prefix, `set '${item.name}' as unreachable, while fetching http://${item.server}:1984/chunk/${+range1 + 1}`)
					}
				}))
			}
...
export const checkBlocked = async (url: string, item: string, server: RangelistAllowedItem) => {
	...
	try {
		...
		if (status >= 500) {
			console.error(prefix, `server ${server} returned ${status} for ${url}. Skipping...`)
			return
		}
		if (status !== 404) {
			...
			slackLogPositive('warning', `[${prefix}] ${item} not blocked on \`${display}\` (status: ${status}), xtrace: '${headers.get('x-trace')}', age: '${headers.get('age')}', content-length: '${headers.get('content-length')}'`)
		} else {
